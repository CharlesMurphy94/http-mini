...
class App extends Component {
  ...
  seeEnemies() {
    getEnemies()
    .then(apiData=>{
      this.setState({armiesArray: apiData})
      // console.log(this.state.armiesArray)
    })
  }

  callTroops() {
    getTroops().then(apiData => {
      this.setState({defensesArray: apiData})
    })
  }
  ...
  handleInput(event) {
    this.setState({newRecruit: event.target.value})
  }


  transformMinion(armyShortname,minionId) {
    patchMinions(armyShortname,minionId)
    .then(() => this.seeEnemies())
  }

  slayLeader(shortname, id) {
    deleteArmy(shortname,id)
    .then(() => this.seeEnemies())
  }
      ...
      <div className="App">
        ...
        <div className="reinforcements">
          <form type="submit">
            ...
            <input id="paperwork" value={this.state.newRecruit} placeholder="Please indicate requested recruit" onChange={(e)=>this.handleInput(e)}/>
